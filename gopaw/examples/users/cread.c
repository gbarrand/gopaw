...
/*function*/ double cread(int K,double RUN[],double ACT[],double EACT[]) {      
      int I,J,IRU;
      float AC,EAC;
      FILE* file;

/*
      printf("debug : cread : K %d\n",K);
*/

      file = fopen("data_activite_radon_gas_trace.dat","rb");
      if(file==NULL) {
        printf("Can't open data_activite_radon_gas_trace.dat\n");
        return 0;
      }
    ...
    /*J = 0;*/
      ...
      while(!feof(file)) {
        if(fscanf(file,"%d %g %g\n",&IRU,&AC,&EAC)!=3) {
          printf("Erreur de lecture de data_activite_radon_gas_trace.dat\n");
          fclose(file);
          return 0;
        }
/*
